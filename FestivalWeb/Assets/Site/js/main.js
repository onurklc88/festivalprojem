jQuery(function ($) {
    ...
    var date = $("#countdown").data('festdate');
    $("#countdown").countdown({ date: date, format: "on" }, function () { });
    ...
    function menuToggle()
    {
        ...
        } else { $('.main-nav').addClass('fixed-menu animated slideInDown'); }
    }
    menuToggle();
    $('#event-carousel, #twitter-feed, #sponsor-carousel ').carousel({ interval: false });
    var form = $('.contact-form'); form.submit(function () {
        'use strict', $this = $(this);
        $.post($(this).attr('action'), function (data) { $this.prev().text(data.message).fadeIn().delay(3000).fadeOut(); }, 'json');
        return false;
    });
